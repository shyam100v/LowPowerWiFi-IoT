# This code is for batch processing ns3 trace and PHY state logs for scenario 4d TCP Tx, PSM, LT LPDS

# Author : Shyam Krishnan Venkateswaran
# Email: shyam1@gatech.edu


# Variable that need to be checked before every run will be tagged with #changeThisBeforeRun
...
csvFileName = 'scenario4d10May2021_noDelACK_1.csv'             #changeThisBeforeRun
...
scenario4dOffset = 25000                # offset time - STA will send PS-POLL after 25 ms after the next beacon of real TCP ACK 
...
stateDict["TCP_WAIT_ACK"] =  stateDict["IDLE"]                                  # Sleep state while waiting for TCP ACK  - for scenario 4c and 4d
stateDict["TCP_TX_RAMPUP"] =  25e-3  
stateDict["TCP_TX_RAMPDOWN"] =  28e-3                               # This will be added after 802.11 ACK Rx - scenario 4c and 4d
stateDict["TCP_ACK_RX_RAMPDOWN"] =  65e-3  
stateDict["TCP_RAMPUP_LPDS_TO_TCP_ACK_RX"] =  20e-3  
# for scenario 4c and 4d
# stateDict["TCP_ACK_RX_RAMPUP"] = 20e-3  
...
durationDict["TCP_TX_rampDown"]             = 6000                                              # This will be added after 802.11 ACK Rx  - scenario 4c and 4d
durationDict["TCP_ACK_RX_rampDown"]         = 1300
durationDict["TCP_LPDS_TO_TCP_ACK_RX_rampUp"]        = 10000            # scenario 4c
# for scenario 4c and 4d
durationDict["TCP_ACK_RX_rampUp"]         = 10000



enableAlwaysOn = False
useCase = 'TCP'
enable_TCP_PSM = False
...
for logFilesCounter in range(logCount):
    ...
    # %%
    # Following block is only for scenario 4d - 
    # Find all TCP ACK RX along with the 802.11 ACK Tx and move it 25 ms after the next beacon
    # The script has to keep track of the total duration and make sure it remains the same

    stateVectorLength = len(stateVectorModified)
    stateVectorBackup = stateVectorModified * 1
    durationVectorBackup = durationVectorModified * 1
    timeStampVectorBackup = timeStampVectorModified * 1
    stateVectorModified = []
    durationVectorModified = []
    timeStampVectorModified = []
    print(f"Total duration before: {np.sum(durationVectorBackup)}")

    oldCounter = 0
    while (oldCounter < stateVectorLength):
        if stateVectorBackup[oldCounter] == "TCP_ACK_RX":
            if (stateVectorBackup[oldCounter + 1] == "IDLE") and (stateVectorBackup[oldCounter + 2] == "ACK_802_11_TX"):
                tcpACKDuration = durationVectorBackup[oldCounter]
                idleDuration = durationVectorBackup[oldCounter + 1]
                ACK80211Duration = durationVectorBackup[oldCounter + 2]
                totalCutDuration = np.sum([tcpACKDuration, idleDuration, ACK80211Duration])
                # Durations are saved - append IDLE to modified vectors
                stateVectorModified.append("IDLE")
                durationVectorModified.append(totalCutDuration)
                timeStampVectorModified.append(timeStampVectorBackup[oldCounter])
                oldCounter = oldCounter + 3     # since we skipped three states
                # Add all atates till and including next beacon Rx
                while (stateVectorBackup[oldCounter] != "BCN_RX"):
                    stateVectorModified.append(stateVectorBackup[oldCounter] )
                    durationVectorModified.append(durationVectorBackup[oldCounter] )
                    timeStampVectorModified.append(timeStampVectorBackup[oldCounter])
                    oldCounter = oldCounter + 1
                # The state is now BCN Rx - append this state as IDLE - this beacon will be skipped
                # stateVectorModified.append(stateVectorBackup[oldCounter] )
                stateVectorModified.append("IDLE" )
                durationVectorModified.append(durationVectorBackup[oldCounter] )
                timeStampVectorModified.append(timeStampVectorBackup[oldCounter])
                oldCounter = oldCounter + 1
                # Beacon Rx has been appended - check if next state is 'IDLE', check if it has scenario4dOffset + totalCutDuration
                if (stateVectorBackup[oldCounter] == "IDLE") and (durationVectorBackup[oldCounter] >= (scenario4dOffset + totalCutDuration)):
                    totalIdleTime = durationVectorBackup[oldCounter]
                    remainingIdleTime = totalIdleTime - scenario4dOffset - totalCutDuration
                    # Add IDLE for scenario4dOffset duration
                    stateVectorModified.append('IDLE')
                    durationVectorModified.append(scenario4dOffset)
                    timeStampVectorModified.append(timeStampVectorBackup[oldCounter])
                    currentTimeStamp = timeStampVectorBackup[oldCounter] + scenario4dOffset
                    # Add the three cut states
                    stateVectorModified.append("TCP_ACK_RX")
                    durationVectorModified.append(tcpACKDuration)
                    timeStampVectorModified.append(currentTimeStamp)
                    currentTimeStamp = currentTimeStamp + tcpACKDuration

                    stateVectorModified.append("IDLE")
                    durationVectorModified.append(idleDuration)
                    timeStampVectorModified.append(currentTimeStamp)
                    currentTimeStamp = currentTimeStamp + idleDuration

                    stateVectorModified.append("ACK_802_11_TX")
                    durationVectorModified.append(ACK80211Duration)
                    timeStampVectorModified.append(currentTimeStamp)
                    currentTimeStamp = currentTimeStamp + ACK80211Duration

                    # Add the remaining IDLE time
                    stateVectorModified.append("IDLE")
                    durationVectorModified.append(remainingIdleTime)
                    timeStampVectorModified.append(currentTimeStamp)

                    oldCounter = oldCounter + 1
                else:
                    print(f"Not enough (or None) IDLE state at {oldCounter}")
                    print(f"State: {stateVectorBackup[oldCounter]}, for duration: {durationVectorBackup[oldCounter]}")


            else:
                print (f"TCP ACK not followed by IDLE and 802.11 ACK at ii = {oldCounter}")
        else:
            # Not a TCP ACK Rx state - just append and increment
            stateVectorModified.append(stateVectorBackup[oldCounter] )
            durationVectorModified.append(durationVectorBackup[oldCounter] )
            timeStampVectorModified.append(timeStampVectorBackup[oldCounter])
            oldCounter = oldCounter + 1
    print(f"Total duration After: {np.sum(durationVectorModified)}")
...
# =============================================================================
    ...
    # durationDict["TCP_TX_rampDown"]
            ...
            # Adding RampDown After 802.11 ACK Tx    
                ...
                # # Adding RampUp - old ----------------- below
                # if (stateVectorRampUpDown[-1] == 'TCP_WAIT_ACK'):
                #     if (durationVectorRampUpDown[-1] >= durationDict["TCP_LPDS_TO_TCP_ACK_RX_rampUp"]): 
                #         durationVectorRampUpDown[-1] = durationVectorRampUpDown[-1] - durationDict["TCP_LPDS_TO_TCP_ACK_RX_rampUp"]
                #         stateVectorRampUpDown.append('TCP_RAMPUP_LPDS_TO_TCP_ACK_RX')
                #         durationVectorRampUpDown.append(durationDict["TCP_LPDS_TO_TCP_ACK_RX_rampUp"])
                #         timeStampVectorRampUpDown.append(timeStampVectorModified[ii] - durationDict["TCP_LPDS_TO_TCP_ACK_RX_rampUp"])
                #     else:
                #         #print(f"Error at ii: {ii}. not enough time for Ramp up from Sleep to TCP ACK Rx. Added {durationVectorRampUpDown[-1]} us instead of {durationDict["TCP_LPDS_TO_TCP_ACK_RX_rampUp"]} us.")
                #         stateVectorRampUpDown[-1] = 'TCP_RAMPUP_LPDS_TO_TCP_ACK_RX'
                # # Adding RampUp - old ----------------- above
                remainingRampUpDuration = durationDict["TCP_LPDS_TO_TCP_ACK_RX_rampUp"]
                temp_stateVector = []
                temp_durationVector = []
                temp_timeStampVector = []
                while (remainingRampUpDuration > 0):
                    if (durationVectorRampUpDown[-1] <= remainingRampUpDuration):
                        remainingRampUpDuration = remainingRampUpDuration - durationVectorRampUpDown[-1]
                        #print(stateVectorRampUpDown)
                        if stateCompare('TCP_RAMPUP_LPDS_TO_TCP_ACK_RX', stateVectorRampUpDown[-1]):
                            stateVectorRampUpDown[-1] = "TCP_RAMPUP_LPDS_TO_TCP_ACK_RX"
                        # Adding the previous state to temp vectors so that they can be popped
                        temp_stateVector.append(stateVectorRampUpDown[-1])
                        temp_durationVector.append(durationVectorRampUpDown[-1])
